<!--
  ...
  disableTunMode 有意不在这里出现:它是给通过一次性链接下发后端的场景用的
  (见 helper/setupHandoff),由分发方决定藏掉哪些功能,
  不是手填地址的人要操心的东西。编辑时原样带过,不清空。

  rememberSecret 相反,是手填地址的人自己的决定:勾了才把控制端密钥落到本地,
  不勾就只留在本次会话里(见 helper/backendCredentialStorage)。所以它属于这张表单。
