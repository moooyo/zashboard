...
// `enabled` comes first because a component usually knows *when* it wants the
// tracking (a sheet that is mounted but closed must not pin the height — that is
// what pushed the sidebar's last tab past the bottom edge) and only rarely wants
// a var other than `--app-height`. Leave it out to track for the whole lifetime.
export const useViewportHeight = (
  ...
  const start = () => {
    if (tracking) return
    tracking = true
    acquireViewportHeight(varName)
  }

  const stop = () => {
    if (!tracking) return
    tracking = false
    releaseViewportHeight(varName)
  }
